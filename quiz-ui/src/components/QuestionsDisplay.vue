<template>
    ...
    <br>
    <div class="font-semibold text-xl ml-6">{{question.text}}</div>
    <br />
    <div class="grid grid-cols-2 gap-12">
        <div v-for="answer, idx in this.question.possibleAnswers" v-bind:key="answer.id">
            <button type="button" @click="chosenAnswer(idx+1)" :class="[this.selectedAnswer === idx+1 ? 'border-green-600 hover:bg-green-400 bg-green-500/50' : 'border-blue-600 hover:bg-blue-400 bg-sky-500/50']" class="w-1/2 px-5 py-5 font-bold col-span-1 ml-6 mr-6 border-2 rounded-lg text-black hover:bg-opacity-40">
                {{ answer.text }}
            </button>
        </div>
    </div>
